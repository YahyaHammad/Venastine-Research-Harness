...
Inside the TUI, `/claims` (or ctrl+l) opens the last research run's claims with their confidence tiers, grounding and assumption flags, and `/copy [last|report|claims|all]` gets text out — Textual 1.0 cannot select text, so `--file <path>` is there for terminals that drop clipboard escapes.

Inside the TUI, `/model <name>` or `/model <PROVIDER> <name>` switches provider and model for the rest of the session (bare `/model` lists which providers have a key). It applies from the next turn and to `/research`, and saves nothing — use the launch flags above, or `default_provider` / `default_model` in `settings.json`, to make a choice stick.
...
ROADMAP.md §1–§12 are fully built (§10's ensemble mode carries a revisit note), as are ROADMAP_v2.md §13 (streaming loop), §14 (config loader + workspace trust), §15 (permission system), §16 (Textual TUI shell, reasoning-effort switching), §17 (MCP client), §18 (agent system: `/agent`, `spawn_subagent`, goal mode, `/grill-me`), §19 (skill system: `/skill`, category folders, four built-in skills), §20 (post-pipeline review: a reviewer agent checks the finished run and proposes corrections you accept, reject or refine), §21a (memory: conversations are condensed automatically as they grow long, with the full history kept intact and unedited; `pin` protects recent turns and `/compact` triggers it by hand), §21b (durable memory: `remember` saves a fact across conversations, scoped to this project or to everything, with your approval each time; `/memories` and `/forget` manage them), §22 (pipeline observability: a research run now reports pass boundaries, claim tiers and retries as they happen, in the TUI's sidebar panel and on the CLI, instead of going silent for several minutes), §25 (authorized tool use in the research pipeline: pre-flight grants, attended mode, argument-side content policy), and §26 (research legibility: a pass now shows the tools it is calling as it calls them, every code-only stage is announced rather than skipped over, `/claims` or ctrl+l opens the run's claims with their tiers and grounding, the transcript is colour-coded by role, and `/copy` gets text out of a terminal that cannot select it). Remaining work is ROADMAP_v2.md §21c (cross-thread referencing, session summaries) and §23–§24: interactive tools and `/init`.

Run the test suite with `pytest` — 1028 tests, fully offline, no API keys needed. One further test is marked `integration` and excluded by default; it spawns a real stdio MCP server (`pytest -m integration`).
