...
| `_within` returns `False` instead of raising on an unresolvable path | Nothing; it was an unhandled crash | `os.path.realpath` raises `OSError: [WinError 668]` on a malformed UNC token such as `\\;`, and `_within` runs inside `classify_command` → `registry.approval_needed()`, which is handed the model's tool input verbatim and has no handler above it. So `ls \\;` crashed the approval gate. The `except` must stay narrow: a bare `except` here reads every token as escaping and turns the whole inert tier into a prompt |
