...
│
├── tests/                          # 4106 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── replay.py                  # ROADMAP_v2 §27: a stored thread as display entries -- the ARCHIVE (T3), tool calls as one redacted line, results skipped (T4). ONE policy, both shells. ReplayEntry is `(role, text, links)` since batch 65 -- a click target is a rendering fact, and without it a resumed thread would draw the same characters as the live turn and refuse to open them
