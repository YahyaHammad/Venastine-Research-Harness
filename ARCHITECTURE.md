...
│
├── tests/                          # 4504 tests, all offline, ~5-15 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_config_edit.py         # 70 tests -- batch 84: /config's catalogue against the schema (every key addressable once, both tool tables expanded, file order not alphabetical), the descriptions generated from the Field constraints, the round trip proved lossless over the WHOLE catalogue -- all 114 settable scalars set to their own values in one document, which is what would catch a leaf written to the wrong table -- the whole-document validation, a CRLF checkout staying CRLF, and main.relaunch_argv including the Windows branch that waits rather than orphaning the terminal
