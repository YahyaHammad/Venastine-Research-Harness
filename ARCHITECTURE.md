...
│
├── tests/                          # 2545 tests, all offline, ~25-45s depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_compaction.py         # 47 tests -- ROADMAP_v2 §21a the trigger (M1/M6), the three fold floors (M4/M5), the compactor run (M2), D27's settings validation, #90's input gate, and #92's compactor-visibility tests
...
cap is the user's settings.json `max_token_budget`. `MODEL_CONTEXT_WINDOWS` feeds
only the pipeline backstop.
