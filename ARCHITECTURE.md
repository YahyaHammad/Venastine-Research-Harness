...
│   ├── widgets.py                 # transcript (Rich Syntax highlighting), raven panels, research progress, GoalBanner; `AgentRow`, `lineage_rows` (the one walk both the panel and ctrl+g's picker draw through, returning each row with the column its own lineage puts it in -- the deeper of that and the row's `depth`, since a research pass adds a display level without adding a spawn level) and an `AgentPanel` whose rows carry the thread they stand for as style metadata (§47) -- the batch 58 mechanism, reused rather than re-derived, because `padding-top: 1` means a content line's index is NOT its y and a click computing one would be free to disagree with the widget that drew it; `ThreadCrumb` draws the stored lineage with every step but the last armed, and both widgets post one `ThreadSelected` rather than reaching into the app; LINKED_ROLES (batch 65) says which lines arm a URL for ctrl+click, and `_links` carries the click target for one the digest truncated
