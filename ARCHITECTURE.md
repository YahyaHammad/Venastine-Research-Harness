...
│
├── tests/                          # 3665 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
**The transcript renders three markdown constructs, and one rule keeps that honest** (batch 53). `tui/markdown.py` owns the grammar -- tables, headings, `**strong**` and `` `code` `` -- and `tui/widgets.py` owns the painting, the same split `tui/diffs.py` and `_render_diff` already keep. A table renders as a `rich.table.Table`, the route `Syntax` already takes: `RichLog.write` accepts any renderable, so column widths and in-cell wrapping stay Rich's problem. Textual's own `Markdown` is a Widget rather than a renderable and cannot go inside a `RichLog` at all.

The rule is `markdown.safe_commit_limit`, which generalises §38's open-fence hold into a CAP over four constructs. `RichLog` appends and cannot rewrite a drawn row, so a fence, a table, a heading or an unclosed `**` committed in halves renders as its own source forever. Paired with it, `_split_committable`'s width rule measures RENDERED cells rather than source characters -- `**bold**` draws four cells narrower than it is written -- and the two are useless apart: the cap is what makes the measurement well-defined, since no cut is offered while a mark is still open. The property both serve is that a streamed answer and the same text replayed by `rerender()` produce identical rows. See AGENTS.md for the four constructs and why detection is GFM-strict.

**Does NOT change:** `_entries` still holds the markdown the model wrote, so `/copy` hands back pipes and a `/theme` replay re-derives the grid; thinking spans stay bar-prefixed prose; the CLI is untouched, since it prints `response.text` whole.
