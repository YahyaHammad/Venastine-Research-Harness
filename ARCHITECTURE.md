...
│
├── tests/                          # 3027 tests, all offline, ~25-45s depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_compaction.py         # 64 tests -- ROADMAP_v2 §21a the trigger (M1/M6), the three fold floors (M4/M5), the compactor run (M2), D27's settings validation, #90's input gate, and #92's compactor-visibility tests. Plus §21's revisit: model-name normalization, the queried window beating the table, and the backstop's actual subtraction boundary -- which nothing had asserted
