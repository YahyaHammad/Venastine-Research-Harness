...
│
├── tests/                          # 3013 tests, all offline, ~25-45s depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_session_overrides.py  # 17 tests -- ROADMAP_v2 §21's revisit (batch 31): /window and /trigger as EPHEMERAL overrides bound to a (provider, model). The two rules that make it safe are both about when an override does NOT apply -- a mismatched pair reads as None without clearing, and /model clears outright so switching back gives the derived value -- plus #89's pin cap following a lowered session trigger
...
│   ├── test_context_window.py     # 15 tests -- ROADMAP_v2 §21's revisit, provider side: the Anthropic window riding along on the effort retrieve (one call, two readings), Google's input_token_limit, the four-alias v1-compatible sniff, and the failure/silence split -- a raised query is warned and NOT cached, a provider that reports nothing is cached because that is an answer
