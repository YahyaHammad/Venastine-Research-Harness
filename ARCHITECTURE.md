...
│
├── tests/                          # 3648 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
**Does NOT change:** `on_prompt_input_submitted`, whose body is untouched; what `/help` prints; the dispatcher, which still reads a `strip()`ped line.
