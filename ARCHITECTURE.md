...
│
├── tests/                          # 4214 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_agent_navigation.py    # 65 tests -- §47 slice 3: the read-only view of a stored run. Its own file because this is the first time #main shows something that is NOT the conversation the session is having, and every failure mode is about that difference -- a live turn writing into the pane being read, a trail that outlives the thread it points into, a poll re-reading a run that has finished, and an escape binding sitting live on a key another widget wants. Clicks are driven through the pilot, so a passing test means the app WIRES the click up
...
│   ├── replay.py                  # ROADMAP_v2 §27: a stored thread as display entries -- the ARCHIVE (T3). ReplayEntry carries a fourth element since §47: the id of the call that opened a run, asked of the registry rather than matched by name, so a resumed spawn line opens what a live one opens, tool calls as one redacted line, results skipped (T4). ONE policy, both shells. ReplayEntry is `(role, text, links)` since batch 65 -- a click target is a rendering fact, and without it a resumed thread would draw the same characters as the live turn and refuse to open them
