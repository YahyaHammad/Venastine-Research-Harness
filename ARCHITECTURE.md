...
│
├── tests/                          # 4203 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_agent_navigation.py    # 59 tests -- §47 slice 3: the read-only view of a stored run. Its own file because this is the first time #main shows something that is NOT the conversation the session is having, and every failure mode is about that difference -- a live turn writing into the pane being read, a trail that outlives the thread it points into, a poll re-reading a run that has finished, and an escape binding sitting live on a key another widget wants. Clicks are driven through the pilot, so a passing test means the app WIRES the click up
