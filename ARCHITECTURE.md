...
│
├── tests/                          # 3997 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_markdown_render.py    # 148 tests -- batch 53: the grammar `_split_fences` declined to be, tested as a pure function (tui/diffs.py's split). Mostly NEGATIVE SPACE, because a false table is not a cosmetic slip -- safe_commit_limit would hold the stream waiting for one that never arrives, so the delimiter row is required and every way of not having one gets a case; plus the cap per construct and the width rule measured in rendered cells
...
│   ├── markdown.py                 # batch 53: what a markdown answer is made of -- tables, headings and the two inline marks. PURE, like diffs.py beside it, so widgets.py can import it. Owns `safe_commit_limit`, the CAP that generalises §38's open-fence hold: RichLog cannot rewrite a drawn row, so no construct is ever committed in halves. INSIDE the package on purpose -- a root markdown.py is the logging.py incident again
