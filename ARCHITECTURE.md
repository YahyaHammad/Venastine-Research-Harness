...
│
├── tests/                          # 4143 tests, all offline, ~2-3 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_compaction.py         # 66 tests -- ROADMAP_v2 §21a the trigger, the three fold floors (M4/M5), the compactor run (M2), D27's settings validation, #90's input gate, and #92's compactor-visibility tests. Plus §21's revisit: model-name normalization, the queried window beating the table, and the backstop's actual subtraction boundary -- which nothing had asserted. Batch 44 reversed M1: the working-set trigger is a SHARE of context_limit() rather than a flat 40k, asserted as a ratio so the test states the rule instead of one model's arithmetic, with settings.json's absolute still outranking it
