...
├── config_edit.py                  # /config's catalogue: what each key takes, what outranks it, the comment-preserving writer, and RestartRequest. Outside tui/ because the TUI is a shell, not a feature home
├── config_update.py                # keeps an edited config.yaml across an npm update: a three-way merge of the user's file against the pristine copy kept in ~/.config/venastine/config-state, run by bin/venastine.mjs before main.py. npm replaces a package directory WHOLESALE, so without this every setting a user had chosen is lost at `npm update`
├── config.py                       # the same values as module attributes, for the 30 modules that read them. A shim: reads config.yaml once at import, binds plain mutable globals, rebuilds the ToolPermissions/ToolApprovals dataclasses
...
│   ├── ROADMAP_v2.md               # §13-§48, all built -- the same, continued. Append-only: never edit a built section's decision record, add a § or a DEVLOG deviation entry instead
│   ├── ROADMAP_v3.md               # §49 onward (batch 93) -- the same record, a third file by owner decision SS21; numbering continues so a § means one section across all three
...
│
├── tests/                          # 4827 tests, all offline, ~5-15 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_config_edit.py         # 80 tests -- batch 84: /config's catalogue against the schema (every key addressable once, both tool tables expanded, file order not alphabetical), the descriptions generated from the Field constraints, the round trip proved lossless over the WHOLE catalogue -- all 113 settable scalars set to their own values in one document, which is what would catch a leaf written to the wrong table -- the whole-document validation, a CRLF checkout staying CRLF, and main.relaunch_argv including the Windows branch that waits rather than orphaning the terminal
│   ├── test_config_update.py     # 47 tests -- batch 86: the three-way merge that carries an edited config.yaml across an npm update (measured against real npm first -- a version bump replaces the file outright, a same-version install re-extracts nothing), the mirror that must never be refreshed before the version check, a container restored together with the prose ruamel anchors to its LAST INDEX, a value the new schema rejects dropped per key rather than aborting the whole merge, the two-way fallback for an install adopted mid-life, and every failure path reported rather than raised
│   ├── test_config_loader.py       # 106 tests -- ROADMAP_v2 §14 frontmatter AC4, tier precedence D8/D18, settings merge, CONTEXT opt-in AC5, catalog, #45's repair-not-reject max_steps; batch 82 the config.yaml coupling -- every compaction default names a real schema field, the shipped file covers the whole schema, and the two permission tables declare the same tools
...
│   ├── test_model_call_retry.py    # 17 tests -- batch 91, the loop's half: a transient model-call failure is retried below the user message and above the accounting -- a drawn run only before its first delta, a drained one whatever it had streamed -- and a run that fails anyway records why on the user row that started it: never for an abandoned generator, never unredacted, and never where provider translation or the compactor reads
│   ├── test_line_pattern.py        # 18 tests -- ROADMAP_v3 §49 (SS8): a monitor pattern the MODEL wrote cannot stall the harness (a backtracking pattern over 200k characters stays under a second; `re` would hold the GIL and freeze every thread), cannot print over the TUI (capfd reads the file descriptor RE2's C++ writes to), and cannot be served by another package's `re2` that falls back to `re`. Plus the refusals the agent is told about, and an AST check that no other module imports re2
│   ├── test_harness_rows.py        # 18 tests -- ROADMAP_v3 §49 (SS5): the row the HARNESS writes to start a turn is an ordinary user message to everything that builds a request (translation identical on all three providers) and plainly not the person to everything that shows or summarises one: replay's `wake` header, the compactor's `harness:` label, the transcript retiring the reply label. Plus the record requirement, the unusable-mark degradation, and `wake_conversation` running under the run's own channel and grant
│   ├── test_session_backends.py    # 41 tests -- ROADMAP_v3 §49: a background session gets the SAME container a one-shot command gets -- the golden one-shot argv, `_route` held against `containment_for` over every tier, runtime answer and fallback posture -- plus exactly the flags a long-lived process needs: `--sig-proxy=false`, the in-container `timeout -k`, the process label, no stdin, merged output, its own signal group. Host sessions, both kill paths (a Windows process tree, a POSIX group TERM then KILL), and exit cleanup scoped to this process's containers
│   ├── test_shell_sessions.py      # 34 tests -- ROADMAP_v3 §49: core/shell_sessions.py with a fake backend and a clock the test moves. The atomic process-wide cap (four threads at the last slot admit one), the one clamp, head + tail paging across the dropped middle, every match waking and matches coalescing per take, a match in output the buffer dropped, the consecutive-wake limit and who resets it, the user's own kill held for their next message but still waking a subagent, bounded close, and a sink called outside the lock
│   ├── test_session_wake.py        # 14 tests -- ROADMAP_v3 §49 (SS5, SS6): what a finished session says to the model. A replay-sized first line, the not-instructions frame, every shape told apart, a coalesced batch sharing one output budget, and REDACTION through the real check_output_policy under the originating tool's name (a spy, because a local copy would pass every other assertion)
│   ├── test_provider_errors.py     # 53 tests -- batch 91: which provider failures are transient, over lookalikes of every SDK family's exceptions (the suite fakes the real ones; the real classes were checked outside pytest), plus retry-after, the backoff arithmetic and describe()'s one line
...
│   ├── test_parallel_calls.py      # 45 tests -- ROADMAP_v2 §47 slice 8 (NA9-NA18): several calls of ONE response running at once. Its own file because the claims are not about one module -- the partition and the conduit are in core/loop.py, the serialised question in core/interaction.py, the budget's lock in core/approval.py, the sink's in tui/app.py and the panel's ordering in tui/widgets.py -- and what they share is the concurrency. EVERY concurrency claim is driven with real threads and a barrier whose timeout IS the assertion, so a sequential implementation fails rather than passing slowly; two of them additionally need the window WIDENED (a long stack and a short switch interval) or SLOWED (an ask that takes as long as a human does), because a race that fits inside one GIL quantum was measured to survive its own test
...
│   ├── test_shell.py               # 288 tests -- ROADMAP §7 sandbox routing, inert/network classification, approval, backend internals; §28 the capability classifier, the modes, the .venastine mount; §48 (batch 92) code in the sandbox asks under `tiered`, `contained` keeping the old rule below the protected-segment check, the fallback opt-in per mode with the badge held against the gate, `runs_code` as its own dimension, and a generative pin that no language is listed; batch 37 the quoting bypass and the protected-path workspace guard; batch 39 the escaping bypass, the generative tokeniser-agreement corpus and the compound-command network flag; §46 INERT routed into the container and HOST_READ left on the host, the approval answer proven unchanged across both containments, `ran_on`/`tier` on every backend, and the container label
│   ├── test_posture.py             # 43 tests -- batch 92 (§48 CE4/CE6) adds the `contained` badge and the fallback pair reported as "no ask" only where the mode honours it; batch 40 (UN1-UN6): the posture is frozen, config/env mutation cannot move it, apply_cli's ordering guard, and every route a session has (settings.json, env, slash command, writing config.yaml) proved not to reach it; batch 82 adds the four that keep that true after the values moved into YAML -- writing config.yaml still needs a human, the file resolves to one place with no env override, and HARNESS_AUTHORITY_KEYS names every unreachable value; batch 84 narrows the slash-command rule to what it was really protecting -- no UNATTENDED route reaches these keys -- and holds /config's confirmation gate in the file about the posture, because a key with no effect sentence is a key whose modal says nothing
...
│   ├── test_permission_context.py  # 22 tests -- ROADMAP_v2 §15 AC1-AC7 (stricter wins, mcp default, redaction survives, D24, unregister) + schemas filtering
│   ├── test_agent_activity.py      # 75 tests -- batch 59's agent activity channel: the span that closes even when the run RAISES, the depth-2 spawn that only reaches a shell because the sink is passed DOWN into the child run, `activity=None` leaving the CLI and the pipeline untouched, and the sidebar panel that draws the stack (indented, truncated to the sidebar's real 18 columns, hidden when idle); batch 67 adds §47's identity -- two runs of one agent at one depth telling themselves apart, the parent read off a ContextVar and restored when a child RAISES, and `bind()` reaching the sink from the production call site rather than from a stub, which is the only test that can see a sidebar row become openable while its run is still going; batch 68 adds the panel's half -- a row armed with the thread it stands for, read back through `get_style_at` because that is the call a click makes, and the peer case last-match removal gets wrong
│   ├── test_agent_navigation.py    # 84 tests -- §47 slice 3: the read-only view of a stored run. Its own file because this is the first time #main shows something that is NOT the conversation the session is having, and every failure mode is about that difference -- a live turn writing into the pane being read, a trail that outlives the thread it points into, a poll re-reading a run that has finished, and an escape binding sitting live on a key another widget wants. Clicks are driven through the pilot, so a passing test means the app WIRES the click up
│   ├── test_agents.py              # 52 tests -- ROADMAP_v2 §18 AC1-AC3 (intersection, depth, manager surface), dispatch injection, headless filter + warning, goal mode, catalog, D24, TUI commands, and R16's "a headless run cannot spawn at all"
...
│   ├── test_shipped_roster.py      # 31 tests -- ROADMAP_v2 §32 A12-A15, batch 51: what the DEFAULT INSTALL ships as a SET. Every declared tool is registered (a whitelist typo silently subtracts, and nothing caught it), read-only asserted through the policy layer for what config ALLOWS and against the declaration for what it already denies, and what a stock install can actually call; A13's superset is SWEPT over every agent that can spawn against everything its spawn_targets let it spawn, both from the declaration and through child_context -- the named two-agent version is what batch 51's expansion broke, by updating plan's comment to claim a superset the whitelist under it never gained
│   ├── test_spawnable.py           # 16 tests -- ROADMAP_v2 §32 A3/A4 (#69): a harness agent that omits `spawnable` is a build error naming every offender, a stranger's silence gets the safe answer, and the default install advertises no spawnable agent at all
│   ├── test_spawn_preflight.py     # 21 tests -- ROADMAP_v2 §32 A7 (#70): one refusal_reason read by the loop, by dispatch ahead of its approval gate, and by run() as the direct-caller backstop -- plus the control that a valid spawn IS still signed off
...
│   ├── test_themes.py             # 319 tests -- batch 29: the roster (grid names unchanged, six standalone tinted themes), role-slot completeness for every theme, and #14's contrast floors (foreground >= 7:1, severity >= 4:1, identity >= 3.5 dark / 3.0 light) computed from the Theme objects; plus #183's restyle wiring and the /theme hint; batch 41 adds the transcript-role pins (X1/X2): pairwise distinctness over MESSAGE_ROLES, a redmean separation floor between the tool and thinking slots, and `dim` confined to the system role; plus X7's diff tint -- readable foreground on it AND a visible band against the page, since blending further satisfies the first floor by failing the feature; batch 64 adds the INTEGRITY half over SELECTABLE_THEMES -- every theme ctrl+p can actually select, derived from BUILTIN_THEMES + ALL_THEMES rather than listed -- asking only whether each one renders: a full role table, no empty slot, and every style string one Rich can PARSE, which is the only check that sees a bad one (Rich resolves an unparseable style with default=Style.null() and says nothing). The quality floors above stay on our fourteen, measured: thirteen of Textual's twenty-one fail a contrast floor, the two ANSI ones cannot be measured at all, and four fail pairwise distinctness. Being DERIVED is what carried that half across the textual 8.2.8 pin move -- the built-in roster grew by nine and every new theme was covered by the run that noticed, which is also where the +21 in this line's count came from
│   ├── test_tui.py                 # 431 tests -- ROADMAP_v2 §16 AC1-AC3 (thread picker, permission round-trip, worker survives a raising tool) + §25 grant picker / attended modal + #106's two axes staying independent + /model's provider/model switch, now remembered for the next launch with its staleness key and its four fallbacks (§43 RM3/RM5) + /new redrawing the window rather than stacking two conversations (RM2) + #172's one-shot notices + §46's rendered-region assertions: every modal must DRAW its body, and the shell command is pinned where it cannot scroll away; batch 49 adds the one they could not make — every DECISION button inside its dialog, over a case table with content long enough to overflow it — plus where focus starts on a consent surface and what it looks like; batch 54 adds the prompt box that wraps and grows, measured on the drawn regions and on max_scroll_x -- because a box that merely got taller may have grown a scrollbar rather than wrapped; batch 55 adds the slash-command suggestion panel, whose width invariant is swept over every registered command WITHOUT a pilot -- the pilot version could not see the wrap-width defect it was written for, because only one command overflows at 80 columns and a bare slash does not show it; batch 56 adds the sliding window, whose budget invariant is swept over every selection at four widths for the same reason; batch 57 adds the two-press ctrl+c and the command aliases, with the armed state asserted on the FOOTER'S DRAWN ROW rather than on the binding -- `check_action` returning None instead of False leaves the dispatch correct and the label stale, and only the drawn row can tell them apart; batch 59 adds TestTheSidebarScrolls -- a Vertical is `overflow: hidden hidden`, so a sidebar holding a checklist and a live research run drew the research panel ENTIRELY off screen, and the two Static panels' own `max-height` + `overflow-y` was EP3's trap a second time: it clips, it does not scroll; batch 62 adds the suggestion panel's key hints, swept at the widths where a part drops rather than at round ones -- the budget is the drawn label's, which is two cells narrower than the width the widget is handed, and only a boundary width can see the difference; batch 66 adds the three ways a MODAL used to kill the app -- a routed WARNING, a startup worker's report and a timeout narration, each driven separately because each failed on its own -- plus the declining default surviving a narration that raises; batch 81 adds `TestShiftEnterIsStillNotTheKey`, which pins the DEPENDENCY facts under a decision rather than the feature it defers -- batch 54's reason for not claiming shift+enter stopped being true at textual 6.6.0 and nobody noticed for twenty months, because it lived in four documents and in no test; batch 84 adds /config's two completion stages and its restart, and re-measures eleven panel tests against the 27-command list -- they walk the bare-slash window by ORDINAL, so a command sorting between `compact` and `copy` moves every one of them
...
│   ├── test_grants.py              # 56 tests -- ROADMAP_v2 §25 R2/R6/R13/R14/R15: grantability, the loop enforcing it, GrantBudget, the audit list, that a grant by name does not answer a subject-carrying question, and that a granted tool is ADVERTISED with no channel (asserted on the wire, since a test that injects the tool call cannot see it); batch 60 adds the two classes that drive the REAL dispatch with only the tool's HANDLER stubbed -- an answered call must actually RUN, and an answer must cover the call it was given for and no other -- plus the structural test that reads core/loop.py, because the two mutations that matter there change no observable behaviour
│   ├── test_attended.py            # 17 tests -- ROADMAP_v2 §25 R9-R11: ApprovalProvider consulted, headless lifted, run-scope declined
│   ├── test_rationale.py           # 26 tests -- ROADMAP_v2 §42 RA2-RA6: shell's display-only `rationale`. The load-bearing one asserts on the arguments the CLASSIFIER RECEIVES rather than on the decision it returns -- an outcome check would pass for an implementation that read the field and happened to agree
...
│   ├── test_diff_view.py           # 34 tests -- ROADMAP_v2 §41 (X4-X7): the block (line numbers, the context window, elision between hunks, the row cap), the GEOMETRY asserted on drawn Strips rather than on text (a changed row marked across its full width, a wrapped row keeping its tint), and app.py's timing -- a denied or refused call draws nothing, which is why the diff is built at the result
│   ├── test_docs_consistency.py    # 35 tests -- the documented counts, ROADMAP_v2's status markers, README's approval table, the decision record's index against its citations, and the npm allowlist (version parity, no secret can ship); batch 59 adds the encoding round trip -- agents/tui_commands.py had been decoded as cp1252 and re-encoded, so it literally contained `Â§` and `â€”` on nine lines, two of them strings the user reads, and nothing else here could see it; batch 80 adds the two that make the FAST GATE able to fail -- both count checks skip on a narrowed invocation and tests.yml runs this file alone, so the gate built to catch drift in seconds was the one invocation that could not see it, and it passed on c5fd7c2 six minutes before the full suite failed on exactly these two tests
│   ├── test_sdk_conformance.py     # 12 tests -- audit #143: the REAL pinned SDKs asked offline (google-genai fields, the thinking_budget pin note, httpx's redirect default). #35's capability chain is asserted here, and driven against real SDK objects -- the test the dict-shaped doubles could not be. Also the three context-window facts §21 assumed away: anthropic's ModelInfo.max_input_tokens, google's Model.input_token_limit, and openai.types.Model being extra="allow" (the property that makes the v1-compatible sniff one function rather than thirteen adapters)
...
│   ├── test_fake_storage_mirror.py # 17 tests -- audit #123: FakeStorage._reconstruct / _split_at compared against storage._to_neutral / _split_at, which two docstrings and AGENTS.md claimed and nothing checked
...
│   ├── test_session_overrides.py  # 20 tests -- ROADMAP_v2 §21's revisit (batch 31), as split in batch 44: /window and /trigger bound to a (provider, model) but no longer sharing a lifetime. The binding is the rule that makes both safe -- a mismatched pair reads as None, so an agent pinning its own model gets its own window -- and the lifetimes are now the interesting half: /trigger is ephemeral and /model clears it, while /window is REMEMBERED in core/model_windows.py because a context window is a fact about a deployment rather than a preference about a session. Plus #89's pin cap following a lowered session trigger
│   ├── test_compaction.py         # 68 tests -- ROADMAP_v2 §21a the trigger, the three fold floors (M4/M5), the compactor run (M2), D27's settings validation, #90's input gate, and #92's compactor-visibility tests. Plus §21's revisit: model-name normalization, the queried window beating the table, and the backstop's actual subtraction boundary -- which nothing had asserted. Batch 44 reversed M1: the working-set trigger is a SHARE of context_limit() rather than a flat 40k, asserted as a ratio so the test states the rule instead of one model's arithmetic, with settings.json's absolute still outranking it
│   ├── test_context_window.py     # 24 tests -- ROADMAP_v2 §21's revisit, provider side: the Anthropic window riding along on the effort retrieve (one call, two readings), Google's input_token_limit, the four-alias v1-compatible sniff, and the failure/silence split -- a raised query is warned and NOT cached, a provider that reports nothing is cached because that is an answer. Batch 44 adds the LISTING fallback: models.retrieve is GET {base}/models/{id} and a `vendor/model` id addresses a route OpenRouter and NVIDIA do not implement, so the one provider the alias comment named as answering was the one that always 404'd
...
│   ├── test_storage_e2e.py     # 49 tests -- ROADMAP_v2 §21a review: real ConversationMemory + real storage.py on real SQLite, compacting four times through the loop path. The only test at this level, and it found the shipped M11 defect. §47's lineage columns are here too, for the same reason: whether `child_threads` returns this parent's children in the right order, and whether the additive migration adds three nullable columns to a database that predates them without backfilling one, are questions about what SQL actually does. §47 slice 8 adds the one measurement nothing else in the suite can take: EIGHT concurrent writers, because at SUBAGENT_MAX_PARALLEL's three a rollback-journal database does not lock and a test at the real ceiling would pass without the fix
...
│   ├── test_thread_legibility.py  # 44 tests -- ROADMAP_v2 §27: what each creation path labels its thread, what the picker is offered, the legacy classification (raw sqlite3), what a replay shows, and the per-thread state a resume must reset
...
│   ├── test_markdown_render.py    # 185 tests -- batch 53: the grammar `_split_fences` declined to be, tested as a pure function (tui/diffs.py's split). Mostly NEGATIVE SPACE, because a false table is not a cosmetic slip -- safe_commit_limit would hold the stream waiting for one that never arrives, so the delimiter row is required and every way of not having one gets a case; plus the cap per construct and the width rule measured in rendered cells; batch 65 adds `link_spans` -- the links-only scanner a tool line gets -- the elision rules that let a truncated URL resolve (one candidate, and the whole ORIGIN visible), and clickable()'s two new refusals: userinfo, and a URL wearing the redaction mark
│   ├── test_history.py            # 22 tests -- batch 63: prompt recall as arithmetic, no pilot. Every case simulates the BOX, because that is what `previous(current)` takes: the value handed in is whatever the last call put on screen, and the whole state machine turns on the two differing. The rule it exists to pin -- that an edit is never destroyed -- is a race between a keystroke and a posted message if it is tested through a Pilot
│   ├── test_meters.py              # 42 tests -- batch 61: the turn meter as arithmetic. NO PILOT AND NO CLOCK, which is what `TurnMeter` taking `now` as a parameter buys -- a live timer tested through a real 0.4s tick either sleeps or flakes. The modal pause is most of the file, because that is the decision (a permission answer is the user's time, not the model's) and because two routes drive it -- the tick's screen-stack read and the permission path's direct call -- which only compose because set_blocked is idempotent
│   ├── test_live_output.py         # 118 tests -- ROADMAP_v2 §38: the commit rules (a completed line drawn without a flush, a long paragraph broken at row boundaries, an open fence held), the ONE entry per streamed span that keeps /copy and /theme honest, the inline thinking block and its collapsed indicator, the app wiring for both forms, and §43's one `venastine ›` per TURN -- above the reasoning rather than after it, above a tool call that OPENS the turn (the MCP shape), never above the pipeline's own `pipeline_tool` lines, and re-derived identically by rerender(); batch 53 adds the table hold (nothing commits mid-table, the prose above it streams anyway, a table open at the flush still draws) and the four constructs' streamed-vs-written row equality, which is what pins the width rule measuring RENDERED cells; batch 65 adds the armed cells of a HARNESS-drawn line -- a tool call, a pipeline row and a failed call -- including the digest shapes the prose grammar would eat, a long URL clickable behind its own truncation, a host that did not fit arming nothing, and the side table surviving a /theme but not a /new
│   ├── test_tui_edge_cases.py      # 12 tests -- TUI rendering edge cases from the `tui-rendering-improvements` review (batches 53-58): a mid-line fence is prose in the splitter and the cap, backslash parity in table cells (double splits, single/triple stay one), the HOLD_LIMIT de-recognition of an over-long mark, stale-alias cleanup on re-register, the thinking split measured in cells, plus the passing guards (rapid double-enter dispatches, persistent selection blocks quit by design)
...
│   ├── test_project_init.py       # 165 tests -- ROADMAP_v2 §24 I1-I17: the two narrow tools, the generated index, stubs vs invented content, one consent covering a named list, and the I6 trust re-grant + §29 N5 the CLI's --init down the response channel + batch 50's `--config`: the template IS the loader's vocabulary, and writing it changes nothing measurable including the log
...
│
├── core/
│   ├── client.py                  # ONE model call, normalized across providers; provider-specific wire formats live ONLY here. call_model_stream() (3 streaming impls) + StreamToken; §33 deleted the non-streaming call_model()/collect_response()/list_models()/select_model() and ModelResponse.raw, none of which had a caller (#38, #39)
...
│   ├── provider_errors.py         # batch 91: which provider failures a model call is retried for (a connection error, 408/409/429/5xx, an error event inside a stream that had already opened), the backoff before a retry, and the one-line description of a failure shared by the transcript, the thread record and the retry warning. Pure and SDK-free: client.py loads SDKs lazily and the suite fakes them, so an exception is read by its MRO class names and the attributes the SDKs define. The attempt itself is core/loop.py's
│   ├── line_pattern.py            # ROADMAP_v3 §49 (SS8): the ONE importer of `re2` -- compiles and matches a monitor session's model-written pattern. Not `re`, which holds the GIL for a whole backtracking match and froze every thread (measured); RE2 is linear and refuses backreferences and lookaround at compile time. Refuses a `re2` from any distribution but google-re2, and keeps RE2's compile errors off stderr, where they would paint over Textual
...
│   ├── replay.py                  # ROADMAP_v2 §27: a stored thread as display entries -- the ARCHIVE (T3). Tool calls as one redacted line, results skipped (T4). ONE policy, both shells. ReplayEntry is `(role, text, links, call_id)`: `links` since batch 65 and `call_id` since §47, both because a click target is a RENDERING fact -- without them a resumed thread draws the same characters as the live turn and quietly refuses to open them. The registry is asked which calls open a thread, rather than the name being matched here, so this file and the TUI cannot disagree about which lines are openable
│   ├── session_wake.py            # ROADMAP_v3 §49 (SS5, SS6): the text and harness record of the row a finished background session starts a turn with. Redacted by the REAL check_output_policy under the originating tool's name, because this text never passes through dispatch -- a local copy would be the second redaction that drifts. Also the held-results row and the killed-at-quit row
│   ├── shell_sessions.py          # ROADMAP_v3 §49 slice 1: the process's one SessionManager. Start (an atomic process-wide cap, the ONE timeout clamp), a supervisor and a reader thread per session, head + tail output IN MEMORY only, RE2 matching on the live stream, per-thread inboxes that coalesce results and stop waking at the consecutive limit, `consuming()` (SS17: only a run something will wake may start one), and a bounded close. No storage import and no widget (D12); snapshots go to its sink outside the lock (NA16)
...
│   ├── agent_activity.py          # Batch 59. WHICH agent is running and how deep -- lifecycle only, never the child's content (§18/D6 returns a distilled answer, not raw history). An out-of-band sink on response_channel's route, because a generator cannot yield from inside registry.dispatch() and run_agent_conversation drains its own loop. What belongs here: the span dataclass, the sink base, the context manager whose `finally` is what stops a row outliving its run. What does NOT: any knowledge of a widget, or of what the agent did.
...
│   ├── untrusted_content          # §32 A8: the injection-defence paragraph, in ONE copy — appended to the preamble above AND to core.loop.DEFAULT_SYSTEM_PROMPT, each with its own closing sentence naming where its instructions come from (#72)
...
│   └── sandbox.py                 # ROADMAP §7 + §28 + ROADMAP_v3 §49: container/subprocess sandbox -- the container from Docker, or Podman when Docker cannot run one, chosen by a once-per-process probe that refuses a Podman unable to enforce the limits -- classify_command(), network allowlist
│
├── safety/
│                                  # batch 41 (X4): `_redact_output_text` -> `redact_output_text`. It always called itself "the one redaction path tool OUTPUT takes"; §41's diff is its second consumer, and `redact_secrets` alone would have redacted less than the digest line the diff replaces
│   ├── __init__.py                # package init
│   └── policy_enforcement.py      # ROADMAP §8 + §25: content-level policy BOTH directions -- blocked domains + secret redaction on results, and refusal on arguments; owns REDACTION_MARKER and, since batch 65, `redacted_values` -- the untruncated candidates behind a digest, with any value the redactor rewrote DROPPED, because a click hands its target to a browser
│
...
│   ├── subagent_tool.py           # spawn_subagent tool (D6 model-initiated); declares parent_context/parent_run for dispatch injection, plus §47's memory/call_id -- what the CHILD's thread records as its parent, so a `▸ spawn_subagent` line can still open its run tomorrow. refusal_reason() settles depth, params, unknown name and (last, so the order stays the live contract) an agent outside the context's spawn_targets
...
│       ├── plan.md                # batch 51: designs an approach before the work starts. NOT spawnable -- its subject is the conversation, which a task string cannot carry. spawn_targets: [explore, review, writer, test] -- it may delegate investigation and NOT implementation, because the plan is the deliverable and a person iterates on it before anything is built. build and general are refused rather than narrowed (C6 would have returned a build that could not write). §32 A13's superset is scoped to that list
...
│   ├── app.py                     # the App -- worker, LoopEvent routing, slash dispatch, permission bridge, both ravens; §18 active-agent state + goal banner. §47 adds the read-only thread view: a ContentSwitcher in #main (the sidebar is OUTSIDE it, which is why it stays live), `_viewing` as the one fact everything else derives from, `_paint_entries` shared with the resume path, and a poll that runs exactly while the viewed run is one the sidebar says is still going
│   │                              # batch 41 (X4): _snapshot() at tool_call_start and _write_file_diff() at a SUCCESSFUL tool_result -- the post-image is reconstructed from the call's own arguments, never read back
│   ├── widgets.py                 # transcript (Rich Syntax highlighting), raven panels, research progress, GoalBanner; `AgentRow`, `lineage_rows` (the one walk both the panel and ctrl+g's picker draw their live rows through, returning each row with the column its own lineage puts it in -- the deeper of that and the row's `depth`, since a research pass adds a display level without adding a spawn level; the picker additionally lists stored descendants below, muted) and an `AgentPanel` whose rows carry the thread they stand for as style metadata (§47) -- the batch 58 mechanism, reused rather than re-derived, because `padding-top: 1` means a content line's index is NOT its y and a click computing one would be free to disagree with the widget that drew it; `ThreadCrumb` draws the stored lineage with every step but the last armed, and both widgets post one `ThreadSelected` rather than reaching into the app; LINKED_ROLES (batch 65) says which lines arm a URL for ctrl+click, and `_links` carries the click target for one the digest truncated
│   │                              # batch 41 (X5): _render_diff -- every row padded and written with an explicit width=, or Strip.adjust_cell_length pads the tail UNSTYLED and the background stops where the text does
...
│   ├── markdown.py                 # batch 53: what a markdown answer is made of -- tables, headings and the two inline marks. PURE, like diffs.py beside it, so widgets.py can import it. Owns `link_spans` (batch 65), the links-only scanner a harness-drawn line gets where prose gets the full grammar -- a tool digest is a shell command or a JSON argument, and the prose scanner does not merely over-decorate one, it EATS CHARACTERS. Owns `safe_commit_limit`, the CAP that generalises §38's open-fence hold: RichLog cannot rewrite a drawn row, so no construct is ever committed in halves. INSIDE the package on purpose -- a root markdown.py is the logging.py incident again
│   ├── meters.py                  # batch 61: what a turn is COSTING while it is spent -- elapsed, an estimated rate, session uptime, and the line written under the finished answer. PURE like markdown.py and diffs.py beside it, and one step further: NO CLOCK either, every method takes `now`, so app.py is the only file that reads a real one and every test here is arithmetic rather than a pilot with a live timer
│   ├── history.py                 # batch 63: the prompts submitted this session, and where a walk back through them has got to. PURE like markdown.py, diffs.py and meters.py beside it -- and like meters.py it takes the thing it would otherwise have to watch: both movers are handed the BOX'S CURRENT TEXT, so an edit made to a recalled prompt is detected by comparison rather than by racing a posted TextArea.Changed, and becomes the draft instead of being walked past
│   ├── diffs.py                   # §41 (X4-X6): what a `write`/`edit` changed, as the canonical text block `Transcript._entries` stores. PURE -- no I/O, no core/ import, so widgets.py can import it; build_block and parse live together so the format has one definition
│   ├── commands.py                # slash-command registry -- MECHANISM only; §18/§19/§21 register into it
│   ├── screens.py                 # ModalScreens: permission prompt (AC2) -- which names the ASKING run since §47, read off the open span and written after the tool's own payload so an agent claim cannot replace a harness fact -- `AgentPickerScreen` (§47's keyboard route to what the sidebar offers on a click), thread picker
│   ├── themes.py                  # 14 themes -- the 8-theme neutral grid (dark/light x plain/red/green/blue) + 6 standalone tinted themes (matrix, nightmare, ember, midnight, glassy-lapis, paper) whose panels carry the identity; role_styles resolves per theme THROUGH _palette (batch 64): every Theme slot but `primary` is optional and four of the twenty-one built-ins the command palette offers leave one blank or fill it with textual's `ansi_*` names, so a blank falls back to textual's own to_color_system().generate() -- a FALLBACK only, since that derivation is lossy and 48x slower, and no shipped theme reaches it -- and `ansi_*` translates into Rich's vocabulary; styles_for contains what is left and warns once per theme, because selecting textual-dark used to kill the app at mount and keep killing it; #14's contrast floors and batch 41's role-separation floor are pinned in tests/test_themes.py
│   ├── preferences.py             # the remembered theme and /model pair -- a USER-tier store beside settings.json (~/.config/venastine/ui_preferences.json), never a write into it; two records, one file, so both writers read-modify-write (§43 RM4)
│   ├── ravens.py                  # mascot art + the tool-name -> activity-state table
│   └── app.tcss                   # styles, driven off theme variables rather than literals
│
└── tools/
    ├── base.py                    # ToolSpec -- the bundle every tool gets registered as; §47 adds `opens_thread`, DECLARED on the tool for grant_policy's reason, since two readers ask it (+ §15 available_check)
    ├── context.py                 # §15: ToolContext -- per-run tool restrictions (allowed_tools / approval_overrides / subagent_depth / spawn_targets). Data only; leaf module, no project imports
...
attributes for the 30 production modules that consume them. The full argument for
...
**`config_edit.py` — the catalogue and the writer** (batch 84). What each key takes
(generated from the schema's own `Field` constraints, so a range that changes there
changes what `/config` offers), what outranks it, the value grammar, the
comment-preserving round trip, and `RestartRequest`.

**Belongs here:** everything `/config` knows that is not drawing. It may import
`core/config_loader` where `config_schema` may not — nothing imports this module
back, so the cycle cannot form from this end. **Does NOT belong here:** anything
about a terminal, and anything that applies a change to a running process.

**Two values per key, and keeping them apart is the point** (batch 85). `KeyRow.in_file` is what `config.yaml` says — what `/config` edits and what the next launch will read — and `in_session` is what this process is running, bound at import and never re-read. A write moves the first without moving the second, so `pending` is the difference, and every number `/config` shows is the file's. One field for both was the defect: the panel, the `(was …)` on a write and the authority modal all reported the LAUNCH value while the write went against the file, so a second change to one key in one session named the wrong value as the one it replaced. `model_name` carries a third fact beside both: the remembered `/model` pair, shown as `remembered` in the panel and expanded into the running sentence by `explain()` — the store is the one tier `catalogue()` cannot otherwise see, and the file/session pair alone reads as the whole precedence.

`pending` excludes an environment override exactly, and needs no startup snapshot to do it: for the five keys with a variable the two legitimately differ from launch, and for every other key they were equal at launch by construction, so a difference IS a change made since. The parsed document is cached on `(mtime_ns, size)` because `catalogue()` runs on every keystroke the panel sees; `write()` clears it outright, since `16000` and `18000` are the same size and two writes inside one filesystem tick would otherwise be invisible.

**`config_update.py` -- the merge that survives an `npm update`** (batch 86). `config.yaml` ships INSIDE the package, and npm replaces a package directory wholesale rather than merging it -- measured with `npm pack` and two installs into a scratch prefix, where an edited `max_tokens: 31337` came back as the shipped `16000`. Moving the file to the home directory would fix that and give away the security property the one-location rule exists for, so the file stays put and this module puts the user's values back into it. `~/.config/venastine/config-state/` keeps the pristine copy of the version that was installed and a mirror of the user's own file; deviations are the one against the other, re-applied onto whatever npm has just written. Keys the user never touched therefore adopt the new version's defaults, and keys the new version ADDS need no code at all -- the merge starts from the new file, so they are already there.

**The state is keyed by the install** (batch 87): `config-state/<12 hex of sha256 of the install directory>/`, the way `runtimePaths()` in the launcher is already keyed by version and requirements hash. One flat directory had a global npm install, a local one and a checkout sharing a single mirror, and whichever ran last overwrote the others' saved settings. The slug is computed HERE AND NOWHERE ELSE -- `installs.json` is how the launcher's doctor line finds it -- because two languages agreeing about a hash by inspection fails silently, writing the mirror where the merge does not look.

Three properties are easy to lose and were each measured. **The mirror is refreshed only after the version check**, because copying `config.yaml` over it first overwrites it with the file npm just replaced -- which is why batch 87 moved the post-session copy out of the launcher and into `--mirror`, where the same guards apply. **A container is restored with the prose that follows it**: ruamel anchors the text after a block sequence to that sequence's LAST INDEX, so replacing the node deleted a section banner and an AUTHORITY note from the file, silently, while every scalar restored cleanly. **Nothing here can stop a launch** -- every failure is a printed report, `main()` returns 0 whatever happened, and the launcher does not check its status.

**Nothing is recorded until the write it describes has happened** (batch 87). `shipped.yaml` used to be copied down before `config_edit.write()`, so a write that raised left the baseline advanced while `version` still named the old release -- and the retry then read every default that had changed between the two as a user edit and pinned it forever, on top of the original failure.

Three further rules. A proposed change is validated as the BYTES it will become — dumped,
re-read through `config_schema.load(path=...)`, which is the candidate seam batch 83
built: no environment overlay, no cache, so a file missing `db_path` is refused even
in a shell where `APP_DB_PATH` would have covered for it. The write preserves
everything it did not change, measured — load and re-dump reproduces all 28,177
bytes, so a one-value change is a one-line diff and the 600 lines of comments
survive. And nothing here applies a change in place: `config.py` binds at import and
`security/posture.py` depends on that (UN1), so a reload would leave the frozen
posture disagreeing with the file it came from.
...
**Belongs here:** `api_initialization()`, `call_model_stream()` (the one model call, with three provider streaming implementations), `ModelResponse`/`ToolCallRequest` (the normalized shapes), the `StreamToken` type (`text_delta` / `final_response`), and the two translation functions `_tools_for_provider()` / `_messages_for_provider()`.

**No longer here (§33, #38/#39):** `call_model()`, `collect_response()`, `list_models()`, `select_model()` and `ModelResponse.raw`. None had a production caller. `list_models` was the sharper one — it took a `client` and returned model ids, so it read as provider-neutral while being OpenAI-shaped (`client.models.list().data`, `model.id`), which made it a third place provider wire knowledge lived, contradicting the contract two paragraphs up. A dead helper that is *wrong* is worse than a dead helper, because it will be reached for.
...
- Tool call IDs: Anthropic and OpenAI always provide an `id` on tool calls. Google's `FunctionCall.id` is optional (defaults to `None`) — `call_model_stream` generates a UUID when missing.
...
**§38's thinking capture, and §44's record of it.** Anthropic iterates the `MessageStream` instead of `stream.text_stream` (one shared underlying iterator, so a caller gets text or both kinds, never text plus thinking) and matches the SDK's synthetic `TextEvent`/`ThinkingEvent`; the OpenAI-compatible branch reads `reasoning_content` or `reasoning` off the delta, which arrive only because `ChoiceDelta` is `extra="allow"`. Google is deliberately unchanged and yields none. Thinking never joins the accumulated TEXT, so `ModelResponse.text` is byte-identical to the pre-§38 one.

**`display: "summarized"` is sent explicitly (§44).** Its default is `"omitted"` on Fable 5, Opus 5, Opus 4.8/4.7 and Sonnet 5, which streams thinking blocks with EMPTY text -- so §38 rendered nothing at all on Anthropic for its whole life, invisibly, because the OpenAI-compatible branch is unaffected.

**`ModelResponse.thinking` carries `{provider, model, blocks}` (§44 WS3/WS4).** The blocks are the PROVIDER'S, read off `get_final_message()` and `model_dump()`'d field for field, because an Anthropic block's `signature` is what the model verifies against its own state -- a record rebuilt from the display deltas would be unsigned. They are persisted in `MessageLog.thinking` and echoed back by `_messages_for_provider` gated on the `(provider, model)` pair, leading the assistant content array; the OpenAI-compatible echo is opt-in per provider (`echoes_reasoning`). A turn that produced none records `None`, so nothing about a pre-§44 thread changes. Both SDK shapes are asserted against the real pinned packages in `tests/test_sdk_conformance.py`, per D22. A stream that completes without yielding a terminal token is an error, raised by `_run()` — that contract used to belong to `collect_response()`, and moved with the code when §33 deleted it.
...
- **`context_window_for(client, provider, model)`** — **queries** the provider for its context window, returning `(window_or_None, authoritative)`. Anthropic is a cache hit off the effort query's own `models.retrieve` (`max_input_tokens` rides on the same `ModelInfo`, so it costs no extra call); Google reads `input_token_limit`; every v1-compatible provider goes through one alias sniff over `.model_extra`, which works because `openai.types.Model` is `extra="allow"`. A query that RAISED is warned and not cached (#35's rule — one transient error must not suppress the query for the life of the process); a query that succeeded and reported nothing IS cached, because that is a real answer. `known_context_window(provider, model)` is the cache-only read `core/compaction.py` uses, which is what lets it ask without holding a client.
...
`ModelResponse.thread_id` is set by `run_deep_research_mode()`, `run_agent_conversation()`, and `continue_conversation()` before they return — never by `call_model_stream()` (which doesn't know which thread it's running against). Callers use it to thread-followup calls (e.g. JSON retries).
...
- **84 test files** (6 per ROADMAP §4 + 2 from §4's own additions: `test_memory_write_through.py`, `test_loop_tool_dispatch.py`; + 2 from §3/§5: `test_json_retry.py`, `test_pipeline_storage.py`; + 2 from §1: `test_cli.py`, `test_e2e.py`; + 1 from §12: `test_output_writer.py`; + 1 from audit: `test_logging_setup.py`; + 1 from §6: `test_file_ops.py`; + 1 from §7: `test_shell.py`; + 1 from §8: `test_policy_enforcement.py`; + 2 from §13: `test_client_streaming.py`, `test_streaming_loop.py`; + 3 from ROADMAP_v2 §14: `test_workspace_trust.py`, `test_config_loader.py`, `test_load_skill.py`; + 1 from §15: `test_permission_context.py`; + 1 from §11: `test_critic_routing.py`; + 1 from §16: `test_client_effort.py`; + 1 from §10's fix: `test_ensemble_guard.py`; + 3 from §17: `test_mcp_client.py`, `test_mcp_config.py`, `test_mcp_integration.py`; + 1 from §16: `test_tui.py`; + 1 from §18: `test_agents.py`; + 4 from §25: `test_grants.py`, `test_attended.py`, `test_research_authorization.py`, `test_granted_calls_artifact.py`; + 1 from §19: `test_skills.py`; + 1 from §20: `test_review.py`; + 1 from the §19–§20 review follow-up: `test_docs_consistency.py`; + 7 from §21a: `test_schema_migration.py`, `test_storage_reads.py`, `test_memory_compaction.py`, `test_compaction.py`, `test_loop_compaction.py`, `test_pin_tool.py`, `test_shell_compaction.py`; + 1 from the §21a review: `test_storage_e2e.py`; + 4 from §21b: `test_memories.py`, `test_remember_tool.py`, `test_memory_injection.py`, `test_memory_shells.py`; + 1 from §22: `test_pipeline_events.py`; + 2 from the 2026-08-05 live-run fixes: `test_tool_failure_containment.py`, `test_truncated_pass.py`; + 1 from §26: `test_research_legibility.py`; + 4 from Audit Pass 1's first fix batch: `test_fake_storage_mirror.py` (#123), `test_prompt_tier_boundary.py` (#146), `test_sdk_conformance.py` (#143), `test_fetch_url.py` (#120/#58); + 3 from §23: `test_interaction.py`, `test_question_tool.py`, `test_todo.py`; + 1 from §24: `test_project_init.py`; + 1 from §21c: `test_thread_refs.py`; + 1 from §27: `test_thread_legibility.py`; + 1 from TECHNICAL_DEBT 8's fix: `test_pilot_wait.py`; + 1 from Audit Pass 1's fifth fix batch: `test_credentials.py` (#19) — the module that writes every provider key to disk, and had no test file at all; + 1 from §31: `test_tool_budgets.py` (#57) — the tool layer had no wall clock at any layer, and no file was about what a call is allowed to cost); + 5 from §32: `test_catalog_advertisement.py` (#68), `test_catalog_text.py` (#131), `test_spawnable.py` (#69), `test_spawn_preflight.py` (#70) and `test_untrusted_content.py` (#72) — unit 8's prompt-assembly boundary had tests for what the catalogs CONTAIN and none for whether the run could act on them); + 1 from §33: `test_tool_arguments.py` (#37) — the call boundary had 81 tests and none for a provider handing back something the translation could not read; + 1 from Audit Pass 1's seventh fix batch: `test_spend_and_size.py` (#4) — item 9's two instruments finally separated: spend is uncapped by default, the only cap is a user setting, and the growth figures live beside the responses they describe; + 1 from Audit Pass 1's eighth fix batch: `test_pid_scanner.py` (#10) — the integration test's own detector, unit-tested from every platform so its ww fix cannot regress blind; + 1 from batch 29: `test_themes.py` (#14 + the standalone-theme roster) — slot completeness and the contrast floors are computed from the Theme objects, so a fifteenth theme inherits the pins for free; + 1 from batch 30: `test_context_window.py` — §21's rejected provider query, rebuilt after its premise was measured false, with the caching rules #35 paid for; + 1 from batch 31: `test_session_overrides.py` — the ephemeral `/window` and `/trigger` overrides, whose whole risk surface is the cases where they must NOT apply; + 1 from batch 40: `test_posture.py` — the frozen security posture, whose assertions are mostly negative space (nothing a session can reach may move it), so every one of them sits beside a positive case proving it moves at all; + 1 from batch 42: `test_rationale.py` — a field whose whole design is what it must NOT reach, so most of the file is negative space and each claim is mutation-checked; + 1 from batch 41: `test_diff_view.py` — the inline `write`/`edit` diff, whose central claim is about GEOMETRY, so it asserts on the drawn Strips rather than on the rendered text; + 1 from batch 63: `test_history.py` -- prompt recall, whose state machine is a walk with a draft saved beside it and therefore arithmetic rather than a pilot; + 1 from batch 53: `test_markdown_render.py` — the block and inline grammar as a pure function, where most of the file is the tables that are NOT there, since a false positive stalls the stream rather than merely rendering oddly; + 1 from batch 74: `test_parallel_calls.py` — §47 slice 8's concurrency, whose claims span five modules and none of which can be asserted without real threads.
...
**Belongs here:** `workspace_trust.py` owns the D17 gate ONLY — `is_trusted()`/`grant_trust()` keyed by resolved project path + a sha256 content hash of everything `content_files()` lists: the root `AGENTS.md` plus everything under `.venastine/`, as PROJECT-relative posix paths fed into the hash with `\0` separators so renames and swaps change the digest. Since §44 `_content_hash()` ITERATES that listing rather than walking a second time, so the trust prompt and the trust digest are one set of files by definition — the module's invariant is that the loader may read no file the listing omits, and two traversals that had to agree by construction is how #18 drifted. `is_trusted()` asks the listing whether there is anything to trust, so a repo carrying an `AGENTS.md` and no `.venastine/` is asked about. `config_loader.py` owns discovery and parsing: line-anchored `---` frontmatter regex (a bare horizontal rule inside a body must not terminate the block — AC4), three-tier `.md` discovery (harness `<root>/{agents,skills}/builtin/` → project `.venastine/` (trust-gated) → user `~/.config/venastine/`), D18 harness-collision rejection with a warning, `settings.json` merge (trusted project > user; unknown keys RAISE; compaction keys are consumed by §21a's effective_compaction(), which also validates the RELATIONSHIPS -- warning_margin < trigger, keep < trigger -- at load rather than leaving incoherent trigger math for later), root `AGENTS.md` caching with per-agent opt-in (`context_for_agent`), and the frontmatter-only `skill_catalog_text()`. Startup cache via `initialize(project_path)` / `reset()`.
...
**The transcript renders a curated markdown grammar, and one rule keeps that honest** (batches 53 and 58). `tui/markdown.py` owns the grammar -- tables, headings, `**strong**`, `` `code` ``, `*emphasis*`, `~~strike~~`, list items and bare URLs -- and `tui/widgets.py` owns the painting, the same split `tui/diffs.py` and `_render_diff` already keep. A table renders as a `rich.table.Table`, the route `Syntax` already takes: `RichLog.write` accepts any renderable, so column widths and in-cell wrapping stay Rich's problem. Textual's own `Markdown` is a Widget rather than a renderable and cannot go inside a `RichLog` at all. What is deliberately absent -- block quotes, horizontal rules, heading LEVELS, markdown inside a reasoning span -- is absent by decision rather than by omission; see AGENTS.md and `TECHNICAL_DEBT.md` 17.

The rule is `markdown.commit_span`, which generalises §38's open-fence hold into a CAP over five constructs. `RichLog` appends and cannot rewrite a drawn row, so a fence, a table, a heading, a list item or an unclosed `**` committed in halves renders as its own source forever. Paired with it, `_split_committable`'s width rule measures RENDERED cells rather than source characters -- `**bold**` draws four cells narrower than it is written -- and the two are useless apart: the cap is what makes the measurement well-defined, since no cut is offered while a mark is still open. The property both serve is that a streamed answer and the same text replayed by `rerender()` produce identical rows.

**Two things bound that cap, and both are batch 58.** `line_start` tells the renderer a chunk begins mid-line, so nothing that depends on where a line starts fires on its first line -- the fix for a shipped defect where a wrap boundary just before a `# ` token made the fragment render as a heading and swallow the hash. `HOLD_LIMIT` bounds the three line-scoped holds so a model that never sends a newline cannot hold the screen; past it the line is prose on BOTH paths, which is what keeps the rows equal. A fence and a table are exempt: both must be drawn whole to be drawn correctly, and `flush_stream` is their bound. See AGENTS.md for the four constructs, why detection is GFM-strict, and why the indent rule is stateless.

**A link's URL is its own label** (batch 58), which is the security posture as a data shape rather than a check: `[text](url)` is not a construct, a bare `http(s)://` URL is marked wherever it appears, and the marked span's TEXT is the target -- so nothing can hide a destination behind a friendly name. Ctrl+click opens it through `Transcript.open_url` on a thread worker, `http`/`https` and ASCII only, with the URL carried in style metadata rather than in textual's parsed `@click` action string.

**Does NOT change:** `_entries` still holds the markdown the model wrote, so `/copy` hands back pipes, hashes, list markers and the brackets of a link, and a `/theme` replay re-derives the grid; thinking spans stay bar-prefixed prose parsed for nothing; `tui/app.py`, `app.tcss` and `PromptInput` are untouched; the CLI is unaffected, since it prints `response.text` whole.

**The prompt box wraps and grows, and it is a `TextArea` for that reason** (batch 54). `PromptInput` in `widgets.py` replaced a plain `Input`, which is single-line by construction; it soft-wraps, grows to four rows via `height: auto` + `max-height: 6` in `app.tcss`, and collapses when the turn is sent. The layout absorbed it unchanged — `#prompt` was already `dock: bottom` above a `1fr` `#transcript`, so the transcript is what pays for the extra rows and gets them back. `enter` submits (`priority=True`, which is what beats `TextArea._on_key`'s own newline insert), `ctrl+j` and `shift+enter` insert a break, and a `value` property over `.text` is what let ~60 existing test sites go untouched. See AGENTS.md for why `shift+enter` cannot be the primary binding and why `alt+enter` is unusable.

**Does NOT change:** the submit handler's body, including its clear-after-the-busy-check rule; the CLI's own input path; the two modal `Input`s in `screens.py`, which stay single-line.

**The slash commands are offered while they are typed, from the registry `/help` reads** (batch 55). `SlashSuggest` in `widgets.py` is a `Static` in `#main`, not a screen: `#prompt` is docked, so the last widget in the flow lands directly above it and the panel "opens upwards" with no layer and no focus to juggle. `CommandRegistry.matching()` supplies the list -- prefix on the name -- so a command registered by §18, §19, §21b or §24 is offered without anything here knowing it exists. Past the first space the COMMAND answers, through an optional `complete(argument)` (batch 84): a command without one offers nothing once a space is typed, exactly as every command did before the field existed, and `/config` is the only one that has one. Its rows are named for the whole line, so `/config max_tokens` draws and completes through the same two methods `/help` does. The panel owns the row budget AND the highlight, because how many entries fit is a function of the rendered width and a prompt that owned the selection could highlight an entry the panel had no room for. `enter` and `tab` complete, the arrows move the highlight over ALL the matches with the window sliding to follow, `escape` latches it shut; all four hand the key straight back when the panel is closed, which is what keeps batch 54's four-row prompt navigable. See AGENTS.md for the API-vs-keyboard seam and the two rendering traps.

**Does NOT change:** `on_prompt_input_submitted`, whose body is untouched; what `/help` prints (batch 57 adds an alias note to one row of it); the dispatcher, which still reads a `strip()`ped line.

**Quitting takes two presses of one key that used to mean four things** (batch 57). `ctrl+c` was `copy` in the prompt — and nothing at all there without a selection — an immediate quit one `tab` away, nothing under a modal, and `copy` again in a modal's `Input`; `README.md` promised it quit. It is now a `priority=True` app binding, which is the only kind that reaches past a focused text widget *and* past `ModalScreen`'s block on app bindings, and the app performs the copy itself because that flag pre-empts `TextArea.copy` outright. The rule is **copy when there is something to copy, arm when there is not**, so no sequence of copies can end a session. The armed state's one visible consequence is the footer: two bindings on `ctrl+c` with different descriptions and `check_action` enabling exactly one, which is textual's dynamic-actions path and the only way a footer label can change at all. `/exit` and `/bye` arrive in the same batch as `SlashCommand.aliases` — a field, so `all()` and `names()` stay canonical and `matching()` offers an alias only once a prefix is typed. See AGENTS.md for why `check_action` must return `False` rather than `None`, and why a bare slash stays the canonical menu.

**Does NOT change:** `PromptInput`, which needs nothing for either half — a priority app binding does not go through the focused widget, and an alias row is an ordinary `SlashCommand`; `app.tcss`; `exit()`, `/quit` and `ctrl+q`, which all still end the session on one invocation; and what a bare `/` offers, which is the same 26 commands in the same order.
...
- **A screen that cannot RENDER never reaches a dismissal path at all** (batch 42, RA1). The rule above was necessary and not sufficient: Textual parses console markup out of any `str` handed to a `Static`, so a square bracket in a tool argument raised `MarkupError` inside `compose()`. The screen was pushed — `isinstance(app.screen, PermissionScreen)` was true — it simply never drew, its dismissal callback never fired, and the worker waited out the same 600s. Every renderable in `tui/screens.py` built from a non-literal is wrapped in `Text(...)`, and an AST walk over that file pins it.
- **A screen that renders can still draw NOTHING, and the layout is what decides** (§46, EP1/EP3). Third rule in the same family, and the one that had been true the longest. `#permission-dialog` sized its middle grid row `1fr` inside a `height: auto` container, and a fractional row in an auto-height container resolves to **zero** — so `#permission-params` had `region.height == 0` and `virtual_size.height == 0` on all four screens sharing that dialog: every tool approval, `SubagentSignoffScreen`'s empty branch, `ConfirmScreen`, and `ProjectKindScreen` with its 277 characters of explanation. Titles and buttons drew; the substance never did. Unlike the two rules above there was no hang and no exception — the modal looked fine and answered correctly, it simply asked its question with the evidence missing. It survived because every assertion about these screens reads `.visual._renderable.plain`, the string the widget was BUILT from, which a zero-height widget still reports in full. One adjacent trap holds: a **`Static`** given `max-height` truncates without scrolling, because its `virtual_size` follows the clamped box rather than the content (`max_scroll_y == 0`, the hidden rows gone rather than below the fold). The other one recorded here — that a scrollable container asked for `height: auto` also resolves to zero — was **false, and batch 49 removed it after it caused the next defect**: the zero belonged to the `1fr` row the box had been measured inside. So a bounded, scrollable block is a `ScrollBox` with `height: auto; max-height: N` wrapping an auto-height `Static`, never a definite height: a definite height bounds what the box DRAWS and not what its parent RESERVES, so the row keeps growing with the content and the dialog's last children — its buttons — are clipped off the bottom. That cost the Allow/Deny row on nine of fourteen modal shapes. `test_every_modal_actually_draws_its_body` walks every screen in `_modal_cases()` and fails any widget that carries text and was allocated no rows; `test_every_modal_keeps_its_decision_on_screen` is the one that can see the buttons, over a case table with content long enough to overflow.
...
**`/model` switches provider and model, and is remembered for the next launch.** The TUI could previously only be pointed at a provider by relaunching it, so a user with no key for `config.MODEL_NAME`'s provider could not reach the shell at all. Three things are load-bearing and each is test-pinned: it **refuses while `_busy`** (swapping under a running worker leaves the transcript claiming one model while the thread records another — the same guard `/new`, `/research` and the thread picker use); it **re-runs the mount-time effort validation**, because effort is per-MODEL and a level the old model accepted can be rejected outright by the new one; and it **warns rather than refuses** on a provider with an empty `API_KEY`, since an OpenAI-compatible endpoint running locally legitimately needs none. It writes nothing to `settings.json` — that file is where a trusted project tier beats the user tier (D29), so a session rewriting it is a decision rather than a convenience, and the project copy is inside D17's trust content hash, so writing `default_model` there would re-trigger the trust prompt at the next launch. Since §43 the pair is remembered BESIDE it instead, in the store `/theme` already uses (RM3): recorded against the `default_provider`/`default_model` it was chosen against, restored whole or not at all, suppressed by a `--provider`/`--model` flag (`cli_pinned`), refused if the provider has left `providers.json`, and announced as `(remembered)` on the mount banner. `App.sub_title` carries the current pair in the header, because the mount banner scrolls away.

**`/theme` and `/model` are remembered across sessions, in a file beside `settings.json` rather than in it.**
The never-writes rule above is about the file, not about preferences in general — so the theme is
persisted to `tui/preferences.py`'s `~/.config/venastine/ui_preferences.json`, at the user tier
only, where no project tier can reach it. Read fail-soft and versioned like `trusted_projects.json`
and `known_mcp_servers.json`, atomic on write, and a failed write WARNS (reaching the transcript
through `TranscriptLogHandler`) and returns False rather than raising, so `/theme`'s confirmation
cannot claim a save that did not happen. The store records the theme *and* the `tui.theme` value it
was chosen against: a remembered choice outranks `settings.json` only while that value is unchanged,
so editing the file re-asserts it instead of being silently outranked forever. Persisting happens in
`watch_theme` rather than in `_cmd_theme`, which is what makes ctrl+p's command palette — including
Textual's own built-in themes — a route that is remembered too; and `_theme_persisting` arms after
the mount assignment, because `App.theme` is a `Reactive` with `init` on and would otherwise record
every launch's restored default as a choice.
...
**The headless callability rule (§15–17 finalization, user-widened; §25 broadened the condition).** `schemas(callable_only=True)` drops any tool whose `approval_needed(name, {}, context)` is True when the run has **no way to ask** — it is uncallable in that configuration, and advertising uncallable tools is the `fetch_url` damage class. `core/loop._run()` passes `callable_only=(response_channel is None)` — **one** condition since §23 merged the two routes into `ResponseChannel`; this read `permission_channel is None and approval_provider is None` while there were two — and logs a WARNING naming `headless_hidden()` output -- deduplicated on the hidden SET, not once per process, because the set is context-dependent and a per-process flag left every later run's larger set silent. autoApproved MCP servers stay advertised because they need no approval.

**What `callable_only` GUARANTEES, and what it does not** (#158). It probes with **empty params**, so for a tool whose `approval_check` reads the params the answer is about a call the model will never make. It means *some* call shape is callable, not that every one is: `write` resolves an empty path inside the workspace, survives the filter, and is then denied for out-of-workspace paths. That is deliberate — an in-workspace write genuinely is callable headless — but it means a survivor of the filter is no promise about a particular call. So `core/loop._denial_reason()` distinguishes *gated by name* (uncallable however called; tell the model to stop) from *gated by these arguments* (tell it to vary them), asked through the same `approval_needed(name, {}, context)` both `dispatch()` and the loop already share. A channel present means somebody was asked and declined, which keeps the original wording: an answer is a decision, not an absence.

**A GRANT is an exception to that rule, and R15 is why** (#156). Both `schemas()` and `headless_hidden()` take `granted=` and share `_answered_by_grant()`: `headless` asks *"can anything ask?"*, but a pre-flight grant is an answer given before the call existed, so the question is *"can anything answer for this tool?"*. A granted tool stays advertised with no channel — which is what `--grant-tools` on an unattended run is for, and it did nothing at all until this. The predicate is `grantable()` **and** `grant_policy == GRANT_ANYWHERE`, re-derived rather than trusted from the set: `ANYWHERE` rather than `!= NEVER` because this path is reached only when the run is headless, which is the unattended case `GRANT_SIGNOFF_ONLY` exists to exclude. The grant **budget** is deliberately not consulted — advertisement is decided once per step, and R6's exhaustion fallback is a call-time answer.
...
specified the latter and it cannot fire: a window-derived threshold sits at a size
a working thread never reaches. The trigger reads measured context size
(`memory.last_input_tokens`) and has NEVER read the billing meter — batch 27 (#4)
made that split explicit: there is no default spend ceiling at all, and the only
cap is the user's settings.json `max_token_budget`. A REMEMBERED window (`core/model_windows.py`, §44) sits above everything below and is checked first, bound to the `(provider, model)` it was set for and kept across launches. Under it, `MODEL_CONTEXT_WINDOWS` is
now the FALLBACK for the window rather than its only source — `context_limit()`
prefers what the provider reported (`core/client.py:context_window_for`, cached
per `(provider, model)` and primed on the path every send crosses) and
normalizes the model id before consulting the table. It feeds the pipeline
backstop **and** `_input_budget()`, which sets the summarizer's one-call ceiling
and gates the lossy truncation in `summarize_thread()` — so a wrong entry costs
discarded thread content, not merely a mistimed backstop.
...
**`/copy` exists because the transcript cannot be selected, and the textual 8.2.8 pin
did NOT change that** — which corrects what stood here. Terminal text selection
arrived at **2.0.0**, not 3.x (`App.ALLOW_SELECT`, `Widget.ALLOW_SELECT`,
`Widget.get_selection`, `Screen.get_selected_text`), and `RichLog.allow_select` has
never existed in any release. `RichLog` implements no `get_selection`, and the default
`Widget.get_selection` extracts from `self._render()` — which for a `ScrollView`
subclass is a debug `rich.panel.Panel`, not a `Text` or `Content`, so it answers
`None`. Making the transcript selectable means writing `Transcript.get_selection`
ourselves; `_entries` and `as_text()` already hold the data, and that is its own batch.
...
4. `RunAgentLoop._run()` begins its loop:
   ...
   c. `call_model_stream(client, provider_name, model, memory.messages, system_prompt, tool_schemas)` — this is where `_messages_for_provider()` and `_tools_for_provider()` translate the neutral shapes into the provider's actual wire format, the SDK call happens, and the raw response is parsed back into a normalized `ModelResponse`.
...
**Sandbox (ROADMAP §7, routing amended by ROADMAP_v2 §46):** `security/sandbox.py` provides hybrid Docker/subprocess isolation. Almost everything runs in Docker (default) or, on explicit opt-in, the subprocess fallback. The ONE exception is tier `HOST_READ` -- a read-only command whose argument resolves outside the workspace -- which takes a lightweight host subprocess because the file it names is one no container can see; it always requires approval, and the prompt names the binary `shutil.which` resolves. Tier `INERT` (every argument inside the workspace) took that same host path until §46 (EP5) and now runs in the container, as an argv rather than through `bash -c`: it is the tier the gate auto-approves, and on Windows the host path was resolving `ls`/`cat`/`grep` off the user's PATH to whichever POSIX toolchain other software had installed. Every result carries `ran_on` (`"container"`/`"host"`) and `tier` (EP4) -- the gate had always told the human where a command would run and never told the model, which is the party that then writes the user an answer about the output. `containment_for()` mirrors this routing and must keep doing so (EP6). Network is disabled by default with a configurable allowlist; containers carry `--label venastine.sandbox=1` and the resolved image ID is logged once per process (EP7).
...
**There is no default token budget anywhere** (batch 27, #4 closed item 9). The cumulative counter in `_run()` is a SPEND meter — the prompt re-bills every step, which is correct as billing and meaningless as a size limit. Both 1M pass/init envelopes existed only because the old 250k chat constant was misread as bounding pass context; with no default cap they had nothing to work around and died. A user who wants a ceiling sets settings.json `max_token_budget` (per `_run()`: one turn, one pass, one /init run); wrappers resolve it through a sentinel so explicit `None` still means genuinely uncapped. Size reasoning reads `ModelResponse.turn_new_tokens`/`.turn_billed_tokens` (unpriced counts) and `memory.last_input_tokens` — never the spend figure.
...
| `shell` | `shell.py` | Working | Shell execution via sandbox module; inert fast-path; Docker default, subprocess fallback. A command whose tokens name a protected segment (`PROTECTED_SEGMENTS`) always requires approval under `tiered` and `contained` — approval-side only, the routing is unchanged. Under `tiered` only an inert command runs unasked (§48 CE1, `CommandProfile.runs_code`); `contained` keeps the pre-§48 rule, contained and no network (CE2) |
...
- **A check that passes by finding nothing cannot tell it looked in fewer places** (#147, batch
  15). The citation check reads comments and docstrings across every production file and passes
  when nothing is unresolved. Narrowing it to comments only — dropping the docstrings, where most
  citations live — left every assertion green. Its own mutation pass scored that as a survivor.
  Same class as batch 14's vacuous tests, reached the other way: there the assertion could not
  discriminate, here the DOMAIN silently shrank. Pin the input in both directions.
- **An id that resolves to two places is a cross-reference that fails silently** (#16, #17, #147,
  batch 15). Six namespaces share the `LETTER+NUMBER` shape. `AC6` meant three different
  acceptance criteria in three files, `D0` a pipeline gate where the record starts at `D1`, and
  `C1`'s only lookupable definition answered a different question. You read the wrong one and it
  makes sense. Every citation now names its namespace, checked by `test_docs_consistency.py`.
- **A comment can be made false by a change somewhere else** (batch 15). `_CITATION`'s `(?<!-)`
  guard was written for `[a-zA-Z0-9]` in a regex literal; once the scan stopped reading code, the
  reason was obsolete while the line stayed correct for a different reason (range endpoints). The
  mutation pass found it, not a reading.
- **Four lists answered one question, and no two agreed** (#96, #94, batch 14). `/init` decided "what counts as a project file" in four places: `manifest._MANIFEST_FILES` (12 names), `detect_facts`' stack branches (7 of those 12), `_root_documents.interesting` (extensions plus `readme*` plus the 12), and `project_docs`' allowlist (4 extensions plus 10 exact names). Every pair disagreed. The five filenames `detect_facts` read no branch for — Gemfile, pom.xml, build.gradle, Makefile, Dockerfile — were exactly the five project shapes proposed as `research` **with `"no code manifests found"` printed as the reason while the manifest listed the manifest**, and the listing advertised six paths the read tool refused while hiding four it accepted. **The general shape: when a value is derived in two places, the bug is not that they drift — it is that nothing can tell you they have.** Neither list was wrong on its own terms; there was no term both were answerable to. The fix is one table and one predicate, so the disagreement has nowhere to live.
- **A test written for the vacuity class was itself vacuous, twice** (#98, batch 14). The batch replaced two tests that could not see the property in their names, and the mutation pass found the same defect in two of the replacements. `os.walk` hands back an already-sorted list on NTFS, so three ordinary filenames could not discriminate a `sorted()` call; and `_first_heading` returns on the first NON-EMPTY line, so a heading buried after 5,000 `x` characters was unreachable whether the 4 KB read cap was there or not. **Both are the ordinary 'input cannot discriminate' shape — the one the sweep is supposed to catch — and what caught them was mutating the line rather than rereading the test.** Carry the practical rule: when a test's assertion depends on an ORDER or a BOUND, the filesystem and the parser have opinions about both, so hold them still (patch `os.listdir`/`os.walk` to answer in reverse; pad with blank lines rather than characters) instead of hoping the environment disagrees with the code.
- **The reachable failure path had no handler, and the unreachable one did** (#95, batch 14). `/init`'s write loop caught `ToolCallDenied`, which cannot fire — consent is granted four lines above — while `_write` raises `InitError` for every error dict `write_run` returns, and `write_run` turns ANY `OSError` into one. So disk full, permissions and a read-only mount all took a path that did not exist: the list of files already created was discarded, and `_settle_trust` was never reached, which left a project the user trusted a moment ago untrusted with nothing saying so. **Worth asking of any `except` clause: what actually raises this, and what raises the thing next to it?** The handler that is present is weak evidence that the failure was thought about.
- **Widening an allowlist is two questions, and only one of them gets asked** (#94, batch 14). Adding `setup.py` to `read_project_doc` sounds like "can a research pass now read arbitrary Python" — no: the matcher is exact-basename OR suffix, so five names were added and `.py` never entered `_DOC_EXTENSIONS`. The question nobody asks is the one that mattered: a basename matches **at any depth**, so it would have meant every `setup.py` in the tree, `src/vendor/setup.py` included. All ten pre-existing entries still behave that way. The five are root-only. **Drive the matcher rather than reasoning about the list**, and check the axis the entry is indexed on, not only the entry.
- **A raising TRANSLATION had none of the containment a raising TOOL has** (#37). `dispatch()` turning a handler's exception into `{"error": ...}` is the entire reason a broken tool cannot kill a run — and nothing played that role one layer up. The accumulated tool-call `arguments` string was parsed with a bare `json.loads`, and `core/loop.py`'s call site sits inside no `try`, so a response cut mid-arguments by the token limit took a `JSONDecodeError` out of `call_model_stream`, out of `_run()`, and into the pipeline's `except Exception` — `status='failed'` on a run that had finished nine passes. **The general shape: containment is a property of a LAYER, not of a function**, so when you find one layer that has it, ask what the layer above it does with the same class of failure. Here the answer was a traceback naming `json.loads` in a file the user has never heard of. Also worth knowing: the parse is OpenAI-compatible only — Google hands back `fc.args` and Anthropic hands back `b.input`, both already dicts — so this is the branch a LOCAL model server takes, which is where truncated tool arguments are likeliest.
- **A test's input could not reach the line the test was named for** (#37, batch 13). `test_finish_reason_survives_the_usage_chunk` asserted that a length cut is still reported once the usage chunk arrives. True, and irrelevant: a usage chunk has `choices: []`, so `if chunk.choices:` skips it and the line under test is never reached. Deleting the mechanism it was written for survived the mutation pass, green. **This is a fourth instance of the vacuity class and a NEW SUB-SHAPE.** §31's `os.times()` (already 0.0 on Windows), #71's `first`/`second` (already sorted) and #69's emptied catalog are all inputs that cannot DISCRIMINATE; this is an input that cannot ARRIVE. The distinction is practical, not taxonomic: the sweep that catches the first three — vary the input and see whether the assertion still holds — does not catch this one, because the assertion DOES hold, for a reason that has nothing to do with the code being tested. What catches it is mutating the line and asking which test should have gone red.
- **Deleting a helper can silently delete a property** (#38, batch 13). `core.client.collect_response()` raised when a stream yielded no final response, and had a test saying so. It had no production caller and was deleted — but `core/loop.py` enforces the same contract, and **nothing covered that copy**, so the obvious tidy-up (delete the helper, delete its test) would have removed the only coverage of a live guard while looking like pure subtraction. When you delete a function, its tests are a list of properties to re-home, not a list of things to remove.
- **A dead helper that is WRONG is worse than a dead helper** (#39). `list_models(client)` took a client and returned model ids, so it read as provider-neutral; it was OpenAI-shaped (`client.models.list().data`, `model.id`), which Google's SDK does not have. That made it a third place provider wire knowledge lived, contradicting the contract AGENTS.md states in the same breath — *"provider wire formats exist in exactly two functions"*. Nothing was broken at runtime and nothing could be: the cost is entirely in what the next person reaches for. Same class as #23's `APICredentials`.
- **Import cost is paid by the commands that use none of it** (#135). `core/client.py` imported all three provider SDKs at module scope; `core.loop` imports it and `main.py` imports `core.loop`, so `--help`, `--summary`, `--ref`, a mistyped flag and the thread picker each paid for three SDKs before argparse ran, of which any one run uses at most one. Paired measurement (`git stash` between runs, AFTERs bracketing the BEFORE, because absolute milliseconds on this machine varied by 2× between sessions): `import core.client` **1766ms → 58ms**, `main.py --help` **2619ms → 1182ms**. **Two things worth carrying:** the import belongs inside the BRANCH that uses it, not at the top of the enclosing function — three of these functions name `genai_types` only on their GOOGLE branch, so the issue's own suggested fix would have kept paying it on every Anthropic turn while looking correct. And the guard test must run in a SUBPROCESS and check the FULL dotted name: the root conftest stubs all three SDKs into `sys.modules` before collection, and `google` alone is a namespace package protobuf puts there regardless — a check on the first segment reports a false positive, which this batch did to itself once while measuring.
- **The prompt told the model to call a tool the same run had just hidden from it, and logged both facts** (#68). `with_catalogs` gated the agent catalog on `registry.is_allowed(name, context)` — POLICY, "is this tool permitted" — while `registry.schemas()` decides advertisement from context AND `callable_only` AND `granted`. A research pass is headless, so the loop dropped `spawn_subagent` from the schema list and named it in that run's own `headless_hidden()` WARNING, while the prompt appended 819 characters saying "The agents below can be spawned with the spawn_subagent tool" — **19% of Pass 1's system prompt**, an instruction the pass could not follow. **The general shape: the schemas are built where the RUN is and the prompt is built before the run exists**, so the catalog asked the only question it could reach from where it stood. A seam, not a mistake in either half — and the fix is not a better condition but ONE predicate (`is_advertised`) with three readers, which is the same remedy `_answered_by_grant` records one method up in the same class. This is review f19's defect on the axis f19 did not cover: its reasoning about advertising uncallable tools was never specific to the context axis, and the headless filter is the axis the pipeline ALWAYS runs on.
- **A fix that empties a collection makes every test asserting absence from that collection vacuous** (#69, batch 12). Declaring all four shipped agents `spawnable: false` emptied the agent catalog, and three assertions in `test_review.py::TestCatalogsFollowTheContext` — written for a different section, about context suppression — would have gone on passing for the wrong reason: *absent because the context suppressed it* and *absent because there was nothing to suppress* are the same observation. Third recorded instance of the vacuity class after §31's `os.times()` (always 0.0 on Windows) and #71's `first`/`second` (already in sorted order), and **the first found by making a change rather than by sweeping for it** — which is the usable part, because the batch doing the emptying is the only moment anyone is positioned to notice. The sibling entry below (batch 6, "a fix that shrinks a set") is the same rule for a smaller set; this is it for an empty one.
- **A description in a file you cloned is prompt text, and the prompt is line-oriented** (#131). Both catalogs render `- {name}: {description}`, and `description` was `str(fm.get("description", ""))` with no length, shape or content check — so a YAML block scalar does not look untidy, it LEAVES ITS BULLET and forges a top-level `## Available tools` section in the system prompt of every run in that project. Reached by one `y` at a trust prompt that listed the file by name and showed 199 characters, none of them the payload. The trust prompt's own comment already stated the criterion — the files shown verbatim are "the ones whose contents change what runs" — and a description meets it exactly; it was excluded because nobody had noticed it qualifies. **Two lessons, and the second is the reusable one:** bound model-facing text from a source you did not write AT THE PRODUCER (arxiv's `MAX_SUMMARY_CHARS` and web_search's `MAX_SNIPPET_CHARS` were already doing this), and a consent screen must show the NORMALISED value — displaying something other than what gets injected invites trust in a string that is not the one used.
- **Clearing an approval flag is not the same as granting the call** (#70, batch 12). The loop's `needs_approval = False` for a call that will be refused anyway only stops the LOOP asking; `dispatch()` re-checks approval itself, finds no callback because nothing asked, and denies — so an unknown agent name came back as `spawn_subagent requires approval and was not given`, a cause that is not the cause and **worse than the empty modal it replaced**. The `is_allowed` precedent one line above works only because dispatch raises its own, correct `ToolCallDenied` first. **The general shape: when you copy a line that suppresses a question, check what answers it downstream** — and the control for "we stopped doing X" is always "the thing X was for still happens", which is why `test_the_refusal_still_reaches_the_model` exists.
...
- **A ladder whose every rung returns the same answer is a pass-through wearing a policy's docstring** (audit #157, §28). `_shell_approval_check` documented five layers. Four were `False` sinks, and the only rung that could return `True` needed a config flag that does not ship — so with `ToolApprovals.shell = False` the set of answers across *every* command shape was `{False}`, on both Docker states. Nobody had measured it because the code reads like a policy and each individual branch is defensible. **The check that finds this is not a review of the branches, it is running the whole function over a spread of inputs and looking at the set of outputs**; a policy whose output set has one element is not a policy. The same probe is worth pointing at any layered check whose layers are `and`ed with config that ships off.
