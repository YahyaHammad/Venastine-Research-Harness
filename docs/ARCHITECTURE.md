...
│
├── tests/                          # 4679 tests, all offline, ~5-15 min depending on the machine (+~5s on the first run for the matplotlib font cache) -- see ROADMAP.md §4, DEVLOG.md §4
...
│   ├── test_shell.py               # 269 tests -- ROADMAP §7 sandbox routing, inert/network classification, approval, backend internals; §28 the capability classifier, the modes, the .venastine mount; §48 (batch 92) code in the sandbox asks under `tiered`, `contained` keeping the old rule below the protected-segment check, the fallback opt-in per mode with the badge held against the gate, `runs_code` as its own dimension, and a generative pin that no language is listed; batch 37 the quoting bypass and the protected-path workspace guard; batch 39 the escaping bypass, the generative tokeniser-agreement corpus and the compound-command network flag; §46 INERT routed into the container and HOST_READ left on the host, the approval answer proven unchanged across both containments, `ran_on`/`tier` on every backend, and the container label
