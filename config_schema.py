...
`config.py` is imported by 31 production modules and ~60 test files, and
`core/config_loader.py` imports it. If the shim reached back into
`core/config_loader` for the values, that would be a cycle -- and `config.py`
was a LEAF module, importing `os` and `dataclasses` and nothing else, which
is the property that made it safe for `security/` to depend on. So the
reader lives here, one hop away, and this module imports the stdlib,
`ruamel.yaml` and `pydantic` and NOTHING first-party. Keep it that way: an
import of anything under `core/`, `security/` or `tools/` from here
reintroduces the cycle by a longer route.
...
class HarnessConfig(_Model):
    ...
    # -- Subagents ---------------------------------------------------------
    subagent_max_depth: StrictInt
    subagent_max_parallel: StrictInt

    # -- Deep research pipeline -------------------------------------------
    max_pipeline_retries: StrictInt
    max_json_retries: StrictInt
    ...
    similarity_calibration: dict[str, dict[str, Number]]
    default_similarity_calibration: dict[str, Number]
    embedder_prefixes: dict[str, dict[str, str]]

    # -- Source authority --------------------------------------------------
    domain_authority_classes: dict[str, Number]
    default_domain_authority: Number
    domain_authority_suffixes: dict[str, str]
    authority_adjustment_cap: Number
    ...
    scholar_api_url: str
    scholar_timeout_s: Number
    scholar_max_retries: StrictInt
    scholar_cache_ttl_s: StrictInt
    scholar_mailto: str
    scholar_venue_weight: Number
    scholar_citation_weight: Number
    scholar_author_weight: Number
    scholar_venue_credit: dict[str, Number]
    scholar_h_saturation: StrictInt
    scholar_min_cohort_size: StrictInt
    scholar_min_citation_age_days: StrictInt
    scholar_retracted_authority: Number
    ...
    max_read_lines: StrictInt
    max_read_chars: StrictInt

    # -- /init -------------------------------------------------------------
    initializer_agent: str
    init_read_chars: StrictInt
    init_max_steps: StrictInt

    # -- MCP teardown ------------------------------------------------------
    teardown_budget_s: Number
    ...
    sandbox_timeout_seconds: StrictInt
    sandbox_memory_mb: StrictInt
    sandbox_cpu_seconds: StrictInt
    sandbox_max_pids: StrictInt

    # -- Output redaction / tool isolation --------------------------------
    redact_tool_outputs: StrictBool
    tool_compute_timeout_s: StrictInt
    network_allowed_commands: list[str]
    inert_commands: list[str]

    # -- Research grants and review ---------------------------------------
    max_granted_tool_calls: StrictInt
    attended_approval_timeout_s: StrictInt
    subagent_review: StrictBool
    max_review_findings: StrictInt
    max_review_refinements: StrictInt

    # -- Compaction --------------------------------------------------------
    compaction_trigger_fraction: Number
    compaction_trigger_tokens: StrictInt
    compaction_warning_margin_tokens: StrictInt
    compaction_keep_recent_tokens: StrictInt
    compaction_keep_recent_turns: StrictInt
    pin_max_trigger_fraction: Number
    compaction_strength: StrictInt
    compaction_max_retries: StrictInt
    compaction_target_ratios: dict[int, Number]
    compaction_ratio_tolerance: Number
    compaction_strategy: CompactionStrategy
    compaction_strategies: tuple[str, ...]
    max_injected_memories: StrictInt

    # -- Catalog text ------------------------------------------------------
    max_catalog_text_chars: StrictInt

    # -- Summaries, references, the checklist -----------------------------
    compactor_agent: str
    summary_target_chars: StrictInt
    max_injected_refs: StrictInt
    todo_statuses: tuple[str, ...]
    max_todo_items: StrictInt
    compaction_pipeline_backstop_tokens: StrictInt

    # -- Context windows ---------------------------------------------------
    model_context_windows: dict[str, StrictInt]
    default_context_window: StrictInt
...
def read_document(path: Optional[str] = None) -> dict:
    ...
    try:
        ...
        raise ValueError(f"{CONFIG_FILENAME} at {target} could not be "
                         f"read: {exc}") from exc
    if not isinstance(data, dict):
        raise ValueError(
            f"{CONFIG_FILENAME} at {target} is not a mapping of settings "
...
def validate(data: dict, source: Optional[str] = None) -> HarnessConfig:
    ...
    try:
        ...
        raise ValueError(
            f"{CONFIG_FILENAME} at {where} is not valid "
...
def tool_permissions_dataclass() -> type:
    return _make_dataclass("ToolPermissions", current().tool_permissions)


def tool_approvals_dataclass() -> type:
    return _make_dataclass("ToolApprovals", current().tool_approvals)
